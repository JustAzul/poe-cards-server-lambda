...
import BuildDynamicFetchListUseCase from './build-dynamic-fetch-list.use-case';
...
export interface FetchFlipDataUseCaseInterfaces {
  ...
  readonly buildDynamicFetchListUseCase: BuildDynamicFetchListUseCase;
}
...
export default class FetchFlipDataUseCase {
  ...
  private async fetchDataForLeague(
    ...
    const { currencyRepository, buildDynamicFetchListUseCase } = this.interfaces;
    const { currencyType = DEFAULT_CURRENCY_TYPE } = this.config;

    const itemTypes = await buildDynamicFetchListUseCase.execute(league);

    const [items, currencies] = await Promise.all([
      this.fetchItemsByLeague(league, itemTypes),
